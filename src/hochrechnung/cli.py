...
def etl(
    ...
    if mode not in ["production", "verification"]:
        console.print(f"[red]Error: Invalid mode '{mode}'. Use 'production' or 'verification'.[/red]")
    ...
    try:
        ...
        if result.mode == "verification" and result.n_flagged_outliers and result.n_flagged_outliers > 0:
            console.print(f"\n[yellow]⚠ Found {result.n_flagged_outliers} flagged counters that need verification[/yellow]")
            console.print(f"[blue]Run verification UI: uv run hochrechnung verify --config {config}[/blue]")
...
def verify(
    ...
    if result.n_flagged_outliers == 0:
        console.print("[green]No flagged counters found. All counters look good![/green]")
        if not prepare_only:
            console.print("[dim]You can still launch the UI to verify unflagged counters[/dim]")
    ...
    try:
        ...
        console.print("[yellow]  Verification UI will work without MBTiles, but tiles won't be visible[/yellow]")
        mbtiles_path = None
    except Exception as e:
    ...
    outlier_result = result.verification_data.get("dtv_volume_ratio").describe() if "dtv_volume_ratio" in result.verification_data else None
    ...
    export_verification_data(
        ...
        median_ratio=result.verification_data.get("dtv_volume_ratio").median() if "dtv_volume_ratio" in result.verification_data else 1.0,
    )
    ...
    if mbtiles_path is None or not mbtiles_path.exists():
        console.print("[yellow]⚠ Warning: MBTiles not available. Map tiles won't be visible.[/yellow]")
